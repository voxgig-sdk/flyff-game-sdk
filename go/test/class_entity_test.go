...
func TestClassEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		classRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.class")))
...
func classBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FLYFF_GAME_TEST_CLASS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFF_GAME_TEST_CLASS_ENTID"])
