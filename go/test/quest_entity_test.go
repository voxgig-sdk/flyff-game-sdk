...
func TestQuestEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Quest(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Quest(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		questRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.quest")))
		...
		questRef01Match := map[string]any{}

		questRef01ListResult, err := questRef01Ent.List(questRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, questRef01ListOk := questRef01ListResult.([]any)
		if !questRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", questRef01ListResult)
		}
...
func questBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FLYFF_GAME_TEST_QUEST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFF_GAME_TEST_QUEST_ENTID"])
