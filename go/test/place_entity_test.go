...
func TestPlaceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_PLACE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func placeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FLYFF_GAME_TEST_PLACE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFF_GAME_TEST_PLACE_ENTID"])
