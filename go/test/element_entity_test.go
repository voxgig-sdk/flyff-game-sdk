...
func TestElementEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_ELEMENT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		elementRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.element", setup.data)))
		var elementRef01Data map[string]any
		if len(elementRef01DataRaw) > 0 {
			elementRef01Data = core.ToMapAny(elementRef01DataRaw[0][1])
		}
		...
		if elementRef01DataDt0Loaded == nil {
...
func elementBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FLYFF_GAME_TEST_ELEMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFF_GAME_TEST_ELEMENT_ENTID"])
