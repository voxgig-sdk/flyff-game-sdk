...
func TestCoreEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FLYFFGAME_TEST_CORE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func coreBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"core01", "core02", "core03", "parameter01", "parameter02", "parameter03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("FLYFFGAME_TEST_CORE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFFGAME_TEST_CORE_ENTID"])
