...
import (
	...
	omni "github.com/voxgig-sdk/flyff-game-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FlyffGameError); is {
				out := map[string]any{
					"name":    "FlyffGameError",
...
func omniCtx(arg any, client *sdk.FlyffGameSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FlyffGameSDK); is {
			client = live
