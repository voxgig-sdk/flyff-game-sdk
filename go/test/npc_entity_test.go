...
func TestNpcEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Npc(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Npc(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		npcRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.npc", setup.data)))
		...
		npcRef01Match := map[string]any{}

		npcRef01ListResult, err := npcRef01Ent.List(npcRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, npcRef01ListOk := npcRef01ListResult.([]any)
		if !npcRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", npcRef01ListResult)
		}
...
func npcBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FLYFF_GAME_TEST_NPC_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFF_GAME_TEST_NPC_ENTID"])
