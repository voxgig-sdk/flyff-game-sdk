...
func TestAchievementEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FLYFFGAME_TEST_ACHIEVEMENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func achievementBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FLYFFGAME_TEST_ACHIEVEMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FLYFFGAME_TEST_ACHIEVEMENT_ENTID"])
