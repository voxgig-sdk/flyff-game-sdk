...
-- by passing it.
function FlyffGameSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FlyffGameSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FlyffGameSDK: graphql: " .. msg
    res.graphql = errors
  end
