...
describe("ClassEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Class(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Class(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_CLASS_ENTID JSON to run live")
      return
    end
...
function class_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FLYFF_GAME_TEST_CLASS_ENTID")
