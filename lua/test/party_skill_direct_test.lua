...
describe("PartySkillDirect", function()
  it("should direct-list-party_skill", function()
    local setup = party_skill_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-party_skill", setup.live and "live" or "unit")
  ...
  it("should direct-load-party_skill", function()
    local setup = party_skill_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-party_skill", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function party_skill_direct_setup(mockres)
