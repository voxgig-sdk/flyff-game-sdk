...
describe("WorldDirect", function()
  it("should direct-list-world", function()
    local setup = world_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-world", setup.live and "live" or "unit")
  ...
  it("should direct-load-world", function()
    local setup = world_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-world", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function world_direct_setup(mockres)
