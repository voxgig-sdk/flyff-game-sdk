...
describe("WorldEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:World(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:World(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:World(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = world_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "world." .. _op, _live and "live" or "unit")
    ...
    local world_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.world")))
    local world_ref01_data = nil
    if #world_ref01_data_raw > 0 then
      world_ref01_data = helpers.to_map(world_ref01_data_raw[1][2])
    end

    -- LIST
    local world_ref01_ent = client:World(nil)
    local world_ref01_match = {}

    local world_ref01_list_result, err = world_ref01_ent:list(world_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(world_ref01_list_result)

    -- LOAD
    local world_ref01_match_dt0 = {
...
function world_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/world/WorldTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read world test data: " .. entity_data_file)
  end
