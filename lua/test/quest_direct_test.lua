...
describe("QuestDirect", function()
  it("should direct-list-quest", function()
    local setup = quest_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-quest", setup.live and "live" or "unit")
  ...
  it("should direct-load-quest", function()
    local setup = quest_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-quest", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function quest_direct_setup(mockres)
