...
describe("ClassDirect", function()
  it("should direct-list-class", function()
    local setup = class_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-class", setup.live and "live" or "unit")
  ...
  it("should direct-load-class", function()
    local setup = class_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-class", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function class_direct_setup(mockres)
