...
describe("HousingTemplateDirect", function()
  it("should direct-list-housing_template", function()
    local setup = housing_template_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-housing_template", setup.live and "live" or "unit")
  ...
  it("should direct-load-housing_template", function()
    local setup = housing_template_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-housing_template", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function housing_template_direct_setup(mockres)
