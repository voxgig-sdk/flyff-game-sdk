...
describe("NpcDirect", function()
  it("should direct-list-npc", function()
    local setup = npc_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-npc", setup.live and "live" or "unit")
  ...
  it("should direct-load-npc", function()
    local setup = npc_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-npc", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function npc_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["FLYFFGAME_TEST_NPC_ENTID"] = {},
