...
local CoupleEntity = {}
CoupleEntity.__index = CoupleEntity


function CoupleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CoupleEntity)
  self._name = "couple"
  self._client = client
...
function CoupleEntity:get_name()
  return self._name
end


function CoupleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CoupleEntity.new(self._client, opts)
end
...
function CoupleEntity:mark_deleted()
  self._deleted = true
end


function CoupleEntity:deleted()
  return true == self._deleted
end


function CoupleEntity:data_set(args)
  if args ~= nil then
...
function CoupleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CoupleEntity:match_set(args)
  if args ~= nil then
...
function CoupleEntity:match_get()
...
function CoupleEntity:stream(action, args, callopts)
...
function CoupleEntity:_run_op(ctx, post_done)
  local utility = self._utility
