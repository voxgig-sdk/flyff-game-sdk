...
local AwakeEntity = {}
AwakeEntity.__index = AwakeEntity


function AwakeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AwakeEntity)
  self._name = "awake"
  self._client = client
...
function AwakeEntity:get_name()
  return self._name
end


function AwakeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AwakeEntity.new(self._client, opts)
end
...
function AwakeEntity:mark_deleted()
  self._deleted = true
end


function AwakeEntity:deleted()
  return true == self._deleted
end


function AwakeEntity:data_set(args)
  if args ~= nil then
...
function AwakeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AwakeEntity:match_set(args)
  if args ~= nil then
...
function AwakeEntity:match_get()
...
function AwakeEntity:stream(action, args, callopts)
...
function AwakeEntity:_run_op(ctx, post_done)
  local utility = self._utility
