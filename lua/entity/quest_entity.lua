...
local QuestEntity = {}
QuestEntity.__index = QuestEntity


function QuestEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QuestEntity)
  self._name = "quest"
  self._client = client
...
function QuestEntity:get_name()
  return self._name
end


function QuestEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QuestEntity.new(self._client, opts)
end
...
function QuestEntity:mark_deleted()
  self._deleted = true
end


function QuestEntity:deleted()
  return true == self._deleted
end


function QuestEntity:data_set(args)
  if args ~= nil then
...
function QuestEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QuestEntity:match_set(args)
  if args ~= nil then
...
function QuestEntity:match_get()
...
function QuestEntity:stream(action, args, callopts)
...
function QuestEntity:_run_op(ctx, post_done)
  local utility = self._utility
