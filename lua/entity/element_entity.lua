...
local ElementEntity = {}
ElementEntity.__index = ElementEntity


function ElementEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ElementEntity)
  self._name = "element"
  self._client = client
...
function ElementEntity:get_name()
  return self._name
end


function ElementEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ElementEntity.new(self._client, opts)
end
...
function ElementEntity:mark_deleted()
  self._deleted = true
end


function ElementEntity:deleted()
  return true == self._deleted
end


function ElementEntity:data_set(args)
  if args ~= nil then
...
function ElementEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ElementEntity:match_set(args)
  if args ~= nil then
...
function ElementEntity:match_get()
...
function ElementEntity:stream(action, args, callopts)
...
---@param reqmatch ElementLoadMatch
---@param ctrl? table
---@return Element
---@return string? err
function ElementEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ElementEntity:_run_op(ctx, post_done)
  local utility = self._utility
