-- FlyffGame SDK PartySkill entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PartySkillEntity = {}
PartySkillEntity.__index = PartySkillEntity


function PartySkillEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PartySkillEntity)
  self._name = "party_skill"
  self._client = client
...
function PartySkillEntity:get_name()
  return self._name
end


function PartySkillEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PartySkillEntity.new(self._client, opts)
end
...
function PartySkillEntity:mark_deleted()
  self._deleted = true
end


function PartySkillEntity:deleted()
  return true == self._deleted
end


function PartySkillEntity:data_set(args)
  if args ~= nil then
...
function PartySkillEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PartySkillEntity:match_set(args)
  if args ~= nil then
...
function PartySkillEntity:match_get()
...
function PartySkillEntity:stream(action, args, callopts)
...
---@param reqmatch PartySkillLoadMatch
---@param ctrl? table
---@return PartySkill
---@return string? err
function PartySkillEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PartySkillListMatch
---@param ctrl? table
---@return PartySkill[]
---@return string? err
function PartySkillEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PartySkillEntity:_run_op(ctx, post_done)
  local utility = self._utility
