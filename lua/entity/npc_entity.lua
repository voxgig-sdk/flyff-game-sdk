...
function NpcEntity.new(client, entopts)
  entopts = entopts or {}
...
function NpcEntity:get_name()
  return self._name
end


function NpcEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NpcEntity.new(self._client, opts)
end
...
function NpcEntity:mark_deleted()
  self._deleted = true
end


function NpcEntity:deleted()
  return true == self._deleted
end


function NpcEntity:data_set(args)
  if args ~= nil then
...
function NpcEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NpcEntity:match_set(args)
  if args ~= nil then
...
function NpcEntity:stream(action, args, callopts)
...
function NpcEntity:_run_op(ctx, post_done)
  local utility = self._utility
