...
local PknEntity = {}
PknEntity.__index = PknEntity


function PknEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PknEntity)
  self._name = "pkn"
  self._client = client
...
function PknEntity:get_name()
  return self._name
end


function PknEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PknEntity.new(self._client, opts)
end
...
function PknEntity:mark_deleted()
  self._deleted = true
end


function PknEntity:deleted()
  return true == self._deleted
end


function PknEntity:data_set(args)
  if args ~= nil then
...
function PknEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PknEntity:match_set(args)
  if args ~= nil then
...
function PknEntity:stream(action, args, callopts)
...
---@param reqmatch PknLoadMatch
---@param ctrl? table
---@return Pkn
---@return string? err
function PknEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PknEntity:_run_op(ctx, post_done)
  local utility = self._utility
