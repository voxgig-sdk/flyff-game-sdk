-- FlyffGame SDK HousingPack entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local HousingPackEntity = {}
HousingPackEntity.__index = HousingPackEntity


function HousingPackEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HousingPackEntity)
  self._name = "housing_pack"
  self._client = client
...
function HousingPackEntity:get_name()
  return self._name
end


function HousingPackEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HousingPackEntity.new(self._client, opts)
end
...
function HousingPackEntity:mark_deleted()
  self._deleted = true
end


function HousingPackEntity:deleted()
  return true == self._deleted
end


function HousingPackEntity:data_set(args)
  if args ~= nil then
...
function HousingPackEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HousingPackEntity:match_set(args)
  if args ~= nil then
...
function HousingPackEntity:match_get()
...
function HousingPackEntity:stream(action, args, callopts)
...
function HousingPackEntity:_run_op(ctx, post_done)
  local utility = self._utility
