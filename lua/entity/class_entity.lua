...
local ClassEntity = {}
ClassEntity.__index = ClassEntity


function ClassEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClassEntity)
  self._name = "class"
  self._client = client
...
function ClassEntity:get_name()
  return self._name
end


function ClassEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClassEntity.new(self._client, opts)
end
...
function ClassEntity:mark_deleted()
  self._deleted = true
end


function ClassEntity:deleted()
  return true == self._deleted
end


function ClassEntity:data_set(args)
  if args ~= nil then
...
function ClassEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClassEntity:match_set(args)
  if args ~= nil then
...
function ClassEntity:match_get()
...
function ClassEntity:stream(action, args, callopts)
...
---@param reqmatch ClassLoadMatch
---@param ctrl? table
---@return Class
---@return string? err
function ClassEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ClassListMatch
---@param ctrl? table
---@return Class[]
---@return string? err
function ClassEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ClassEntity:_run_op(ctx, post_done)
  local utility = self._utility
