...
function WorldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorldEntity)
  self._name = "world"
  self._client = client
...
function WorldEntity:get_name()
  return self._name
end


function WorldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorldEntity.new(self._client, opts)
end
...
function WorldEntity:mark_deleted()
  self._deleted = true
end


function WorldEntity:deleted()
  return true == self._deleted
end


function WorldEntity:data_set(args)
  if args ~= nil then
...
function WorldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorldEntity:match_set(args)
  if args ~= nil then
...
function WorldEntity:match_get()
...
function WorldEntity:stream(action, args, callopts)
...
---@param reqmatch WorldLoadMatch
---@param ctrl? table
---@return World
---@return string? err
function WorldEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch WorldListMatch
---@param ctrl? table
---@return World[]
---@return string? err
function WorldEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function WorldEntity:_run_op(ctx, post_done)
  local utility = self._utility
