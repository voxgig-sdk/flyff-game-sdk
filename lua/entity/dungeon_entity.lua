...
local DungeonEntity = {}
DungeonEntity.__index = DungeonEntity


function DungeonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DungeonEntity)
  self._name = "dungeon"
  self._client = client
...
function DungeonEntity:get_name()
  return self._name
end


function DungeonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DungeonEntity.new(self._client, opts)
end
...
function DungeonEntity:mark_deleted()
  self._deleted = true
end


function DungeonEntity:deleted()
  return true == self._deleted
end


function DungeonEntity:data_set(args)
  if args ~= nil then
...
function DungeonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DungeonEntity:match_set(args)
  if args ~= nil then
...
function DungeonEntity:match_get()
...
function DungeonEntity:stream(action, args, callopts)
...
---@param reqmatch DungeonLoadMatch
---@param ctrl? table
---@return Dungeon
---@return string? err
function DungeonEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DungeonEntity:_run_op(ctx, post_done)
  local utility = self._utility
