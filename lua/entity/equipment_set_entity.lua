...
local EquipmentSetEntity = {}
EquipmentSetEntity.__index = EquipmentSetEntity


function EquipmentSetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EquipmentSetEntity)
  self._name = "equipment_set"
  self._client = client
...
function EquipmentSetEntity:get_name()
  return self._name
end


function EquipmentSetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EquipmentSetEntity.new(self._client, opts)
end
...
function EquipmentSetEntity:mark_deleted()
  self._deleted = true
end


function EquipmentSetEntity:deleted()
  return true == self._deleted
end


function EquipmentSetEntity:data_set(args)
  if args ~= nil then
...
function EquipmentSetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EquipmentSetEntity:match_set(args)
  if args ~= nil then
...
function EquipmentSetEntity:match_get()
...
function EquipmentSetEntity:stream(action, args, callopts)
...
---@param reqmatch EquipmentSetLoadMatch
---@param ctrl? table
---@return EquipmentSet
---@return string? err
function EquipmentSetEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EquipmentSetListMatch
---@param ctrl? table
---@return EquipmentSet[]
---@return string? err
function EquipmentSetEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function EquipmentSetEntity:_run_op(ctx, post_done)
  local utility = self._utility
