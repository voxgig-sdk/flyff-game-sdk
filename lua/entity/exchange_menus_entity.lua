-- FlyffGame SDK ExchangeMenus entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ExchangeMenusEntity = {}
ExchangeMenusEntity.__index = ExchangeMenusEntity


function ExchangeMenusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ExchangeMenusEntity)
  self._name = "exchange_menus"
  self._client = client
...
function ExchangeMenusEntity:get_name()
  return self._name
end


function ExchangeMenusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ExchangeMenusEntity.new(self._client, opts)
end
...
function ExchangeMenusEntity:mark_deleted()
  self._deleted = true
end


function ExchangeMenusEntity:deleted()
  return true == self._deleted
end


function ExchangeMenusEntity:data_set(args)
  if args ~= nil then
...
function ExchangeMenusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ExchangeMenusEntity:match_set(args)
  if args ~= nil then
...
function ExchangeMenusEntity:match_get()
...
function ExchangeMenusEntity:stream(action, args, callopts)
...
---@param reqmatch ExchangeMenusLoadMatch
---@param ctrl? table
---@return ExchangeMenus
---@return string? err
function ExchangeMenusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ExchangeMenusEntity:_run_op(ctx, post_done)
  local utility = self._utility
