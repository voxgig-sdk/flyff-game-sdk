...
from flyffgame_sdk.flyffgame_types import (
    HousingTemplate,
...
class HousingTemplateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return HousingTemplateEntity(self._client, opts)
    ...
    def data_get(self) -> HousingTemplate:
    ...
    def match_get(self) -> HousingTemplate:
    ...
    def load(self, reqmatch=None, ctrl=None) -> HousingTemplate:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.HousingTemplate().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[HousingTemplate]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.HousingTemplate().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
