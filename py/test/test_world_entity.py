...
class TestWorldEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.World(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.World(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _world_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "world." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FLYFF_GAME_TEST_WORLD_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        world_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.world")))
        world_ref01_data = None
        if len(world_ref01_data_raw) > 0:
        ...
        world_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(world_ref01_data_dt0_loaded))
        assert world_ref01_data_dt0_load_result is not None
        assert world_ref01_data_dt0_load_result["id"] == world_ref01_data["id"]



def _world_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/world/WorldTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FLYFF_GAME_TEST_WORLD_ENTID"))
