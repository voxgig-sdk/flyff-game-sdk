...
class TestCoupleEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Couple(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _couple_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "couple." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FLYFF_GAME_TEST_COUPLE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        couple_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.couple")))
        couple_ref01_data = None
        if len(couple_ref01_data_raw) > 0:
            couple_ref01_data = helpers.to_map(couple_ref01_data_raw[0][1])

        # LOAD
        couple_ref01_ent = client.Couple(None)
        couple_ref01_match_dt0 = {}
        couple_ref01_data_dt0_loaded = couple_ref01_ent.load(couple_ref01_match_dt0, None)
        assert couple_ref01_data_dt0_loaded is not None



def _couple_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/couple/CoupleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FLYFF_GAME_TEST_COUPLE_ENTID"))
