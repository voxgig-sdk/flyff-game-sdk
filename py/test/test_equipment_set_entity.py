...
class TestEquipmentSetEntity:

    def test_should_create_instance(self):
        testsdk = FlyffGameSDK.test(None, None)
        ent = testsdk.EquipmentSet(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.EquipmentSet(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.EquipmentSet(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _equipment_set_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "equipment_set." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FLYFF_GAME_TEST_EQUIPMENT_SET_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        equipment_set_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.equipment_set")))
        equipment_set_ref01_data = None
        if len(equipment_set_ref01_data_raw) > 0:
...
def _equipment_set_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/equipment_set/EquipmentSetTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FLYFF_GAME_TEST_EQUIPMENT_SET_ENTID"))
