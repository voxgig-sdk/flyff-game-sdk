...
class FlyffGameTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FlyffGameTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FlyffGameTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FlyffGameTestRunner.getenv("FLYFF_GAME_TEST_LIVE")
        override = FlyffGameTestRunner.getenv("FLYFF_GAME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FlyffGameTestRunner.getenv(key)
        ...
        explain = FlyffGameTestRunner.getenv("FLYFF_GAME_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FLYFF_GAME_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FlyffGameTestRunner._test_control is not None:
            return FlyffGameTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FlyffGameTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FlyffGameTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FlyffGameTestRunner.load_test_control()
...
def load_env_local():
    FlyffGameTestRunner.load_env_local()


def env_override(m):
    return FlyffGameTestRunner.env_override(m)


def entity_data(v):
    return FlyffGameTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FlyffGameTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FlyffGameTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FlyffGameTestRunner.load_test_control()


def live_client_options():
    return FlyffGameTestRunner.live_client_options()


def live_delay_ms():
    return FlyffGameTestRunner.live_delay_ms()
