...
class TestHousingTemplateEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.HousingTemplate(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.HousingTemplate(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID JSON to run live")
        client = setup["client"]
...
def _housing_template_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID"))
