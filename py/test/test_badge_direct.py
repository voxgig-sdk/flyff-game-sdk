...
class TestBadgeDirect:

    def test_should_direct_load_badge(self):
        setup = _badge_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-badge", "live" if setup["live"] else "unit")
        if _skip:
...
def _badge_direct_setup(mockres):
