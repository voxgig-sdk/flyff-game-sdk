...
class HousingPackEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FlyffGameSDK::test(null, null);
        $ent = $testsdk->HousingPack(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->HousingPack(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->HousingPack(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = housing_pack_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "housing_pack." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_HOUSING_PACK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $housing_pack_ref01_data = null;
        if (count($housing_pack_ref01_data_raw) > 0) {
...
function housing_pack_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/housing_pack/HousingPackTestData.json';
    ...
    $entid_env_raw = getenv("FLYFF_GAME_TEST_HOUSING_PACK_ENTID");
