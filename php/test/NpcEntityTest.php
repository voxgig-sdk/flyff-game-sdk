...
class NpcEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FlyffGameSDK::test(null, null);
        $ent = $testsdk->Npc(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Npc(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Npc(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = npc_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "npc." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_NPC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function npc_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/npc/NpcTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["npc01", "npc02", "npc03"] as $k) {
    ...
    $entid_env_raw = getenv("FLYFF_GAME_TEST_NPC_ENTID");
