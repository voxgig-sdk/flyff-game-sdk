...
class PartySkillEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FlyffGameSDK::test(null, null);
        $ent = $testsdk->PartySkill(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->PartySkill(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->PartySkill(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = party_skill_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "party_skill." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_PARTY_SKILL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $party_skill_ref01_data = null;
        if (count($party_skill_ref01_data_raw) > 0) {
...
function party_skill_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/party_skill/PartySkillTestData.json';
    ...
    $entid_env_raw = getenv("FLYFF_GAME_TEST_PARTY_SKILL_ENTID");
