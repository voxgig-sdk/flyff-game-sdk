...
class VersionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_VERSION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function version_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FLYFF_GAME_TEST_VERSION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FLYFF_GAME_TEST_VERSION_ENTID"]);
