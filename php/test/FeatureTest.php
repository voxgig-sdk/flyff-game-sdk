...
class FtCtrl extends FlyffGameControl
{
...
class FtClient
{
    ...
    private FlyffGameUtility $utility;

    public function __construct(FlyffGameUtility $utility, array $options)
    {
    ...
    public function get_utility(): FlyffGameUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FlyffGameContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FlyffGameContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FlyffGameContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FlyffGameConfig::make_feature($name);
    ...
    private static function buildUrl(FlyffGameSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FlyffGameError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FlyffGameResponse($fetched);
        }
    ...
    private function fail(FlyffGameContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FlyffGameContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
