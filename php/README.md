...
$client = FlyffGameSDK::test([
    "entity" => ["badge" => ["test01" => ["id" => "test01"]]],
]);

// Entity ops return the ENTITY (throws on error);
// call data_get() for the mock record.
$badge = $client->Badge()->load(["id" => "test01"]);
print_r($badge->data_get());
```
...
| `autoAttackFactors` | `array` | Auto attack damage factors used in damage calculation |
...
| `mp` | `float` | MP Factor |
| `name` | `array` | Text available in several languages |
