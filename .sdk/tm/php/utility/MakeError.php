...
class FlyffGameMakeError
{
    public static function call(?FlyffGameContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FlyffGameContext([], null);
        }
        $op = $ctx->op ?? new FlyffGameOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FlyffGameResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FlyffGameError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FlyffGameError('', $msg, $ctx);
        ...
        if ($err instanceof FlyffGameError) {
