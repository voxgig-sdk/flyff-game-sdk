...
// FlyffGame SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class FlyffGameMakePoint
{
    public static function call(FlyffGameContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof FlyffGameError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = FlyffGameHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = FlyffGameHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
