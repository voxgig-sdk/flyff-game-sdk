...
{
    private static function utility(): FlyffGameUtility
    {
        return new FlyffGameUtility();
    }
    ...
    private static function ctx(array $over = []): FlyffGameContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FlyffGameContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FlyffGameOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FlyffGameError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FlyffGameOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FlyffGameMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FlyffGameOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FlyffGameMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FlyffGameOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FlyffGameMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FlyffGameSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FlyffGameMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FlyffGameResponse([]),
            'result' => new FlyffGameResult([]),
        ]);
        [, $err] = FlyffGameMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FlyffGameSpec([]),
            'response' => null,
            'result' => new FlyffGameResult([]),
        ]);
        [, $err] = FlyffGameMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FlyffGameSpec(['step' => 's']),
            'response' => new FlyffGameResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FlyffGameResult([]),
        ]);
        [, $err] = FlyffGameMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FlyffGameSpec(['step' => 's']),
            'response' => new FlyffGameResponse(self::resp(200, ['v' => 1])),
            'result' => new FlyffGameResult([]),
        ]);
        [, $err] = FlyffGameMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FlyffGameSpec(['step' => 's']),
            'response' => new FlyffGameResponse(self::resp(200, ['v' => 2])),
            'result' => new FlyffGameResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FlyffGameMakeResponse::call($ctx);
    ...
    {
        $preset = new FlyffGameResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FlyffGameSpec([]),
            'response' => new FlyffGameResponse([]),
            'result' => new FlyffGameResult([]),
        ...
        [$got, $err] = FlyffGameMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FlyffGameResult([])]);
        [, $err] = FlyffGameMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FlyffGameSpec([]), 'result' => null]);
        [, $err] = FlyffGameMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FlyffGameSpec(['step' => 's']),
            'result' => new FlyffGameResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FlyffGameOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FlyffGameMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FlyffGameSpec(['step' => 's']),
            'result' => new FlyffGameResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FlyffGameOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FlyffGameMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FlyffGameResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FlyffGameSpec([]),
            'result' => new FlyffGameResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FlyffGameMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FlyffGameMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FlyffGameError('boom', 'boom');
        $utility->fetcher = function (FlyffGameContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FlyffGameSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FlyffGameMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FlyffGameContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FlyffGameMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FlyffGameResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FlyffGameContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FlyffGameSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FlyffGameMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FlyffGameResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FlyffGameSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FlyffGameMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FlyffGameMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FlyffGameMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FlyffGameResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FlyffGameDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FlyffGameResult(['ok' => false])]);
        $this->expectException(FlyffGameError::class);
        FlyffGameDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FlyffGameResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FlyffGameMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FlyffGameMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FlyffGameBaseFeature();
        $b = new FlyffGameBaseFeature();
        FlyffGameFeatureAdd::call($ctx, $a);
        FlyffGameFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FlyffGameBaseFeature
    {
        $f = new FlyffGameBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FlyffGameFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FlyffGameContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FlyffGameSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FlyffGamePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FlyffGameResponse(['headers' => null]),
            'result' => new FlyffGameResult([]),
        ]);
        FlyffGameResultHeaders::call($ctx);
