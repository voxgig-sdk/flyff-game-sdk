...
module FlyffGameUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FlyffGameResponse.new({})
    ...
    if fetch_err
      ...
      response = FlyffGameResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FlyffGameResponse.new(fetched)
    else
