...
class HousingPackEntity
  ...
  def make
    opts = @_entopts.dup
    HousingPackEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.HousingPack.load works with no args).
  ...
  #
  # @param reqmatch [HousingPackListMatch, Hash, nil] match filter (any subset of
  #   HousingPack fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
