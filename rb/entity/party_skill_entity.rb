...
class PartySkillEntity
  ...
  def make
    opts = @_entopts.dup
    PartySkillEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.PartySkill.load works with no args).
  ...
  #
  # @param reqmatch [PartySkillListMatch, Hash, nil] match filter (any subset of
  #   PartySkill fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
