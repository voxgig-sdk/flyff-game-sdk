...
class HousingTemplateEntity
  ...
  def make
    opts = @_entopts.dup
    HousingTemplateEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FlyffGameHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [HousingTemplate, Hash] the current HousingTemplate data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FlyffGameHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of HousingTemplate fields)
  def match_get
  ...
  #
  # @param reqmatch [HousingTemplateLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.HousingTemplate.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [HousingTemplate, Hash] the loaded HousingTemplate; raises FlyffGameError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FlyffGameHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [HousingTemplateListMatch, Hash, nil] match filter (any subset of
  #   HousingTemplate fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<HousingTemplate>, Array] the matching HousingTemplate items; raises FlyffGameError on failure
