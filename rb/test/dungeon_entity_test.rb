...
class DungeonEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = dungeon_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "dungeon." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_DUNGEON_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    dungeon_ref01_data = nil
    if dungeon_ref01_data_raw.length > 0
...
def dungeon_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "dungeon", "DungeonTestData.json")
  ...
  entid_env_raw = ENV["FLYFF_GAME_TEST_DUNGEON_ENTID"]
