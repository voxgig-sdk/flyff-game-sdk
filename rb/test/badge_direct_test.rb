...
class BadgeDirectTest < Minitest::Test
  def test_direct_load_badge
    setup = badge_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-badge", setup[:live] ? "live" : "unit")
    if _should_skip
...
def badge_direct_setup(mockres)
