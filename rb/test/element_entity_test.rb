...
class ElementEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = element_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "element." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FLYFFGAME_TEST_ELEMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    element_ref01_data = nil
    if element_ref01_data_raw.length > 0
    ...
    element_ref01_data_dt0_loaded, err = element_ref01_ent.load(element_ref01_match_dt0, nil)
    assert_nil err
...
def element_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "element", "ElementTestData.json")
  ...
  entid_env_raw = ENV["FLYFFGAME_TEST_ELEMENT_ENTID"]
