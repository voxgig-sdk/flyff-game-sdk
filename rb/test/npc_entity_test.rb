...
class NpcEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Npc(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Npc(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = npc_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "npc." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_NPC_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    npc_ref01_data = nil
    if npc_ref01_data_raw.length > 0
...
def npc_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "npc", "NpcTestData.json")
  ...
  entid_env_raw = ENV["FLYFF_GAME_TEST_NPC_ENTID"]
