...
class HousingTemplateEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.HousingTemplate(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.HousingTemplate(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = housing_template_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "housing_template." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    housing_template_ref01_data = nil
    if housing_template_ref01_data_raw.length > 0
...
def housing_template_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "housing_template", "HousingTemplateTestData.json")
  ...
  entid_env_raw = ENV["FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID"]
