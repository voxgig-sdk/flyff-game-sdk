...
class EquipmentSetEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.EquipmentSet(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.EquipmentSet(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = equipment_set_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "equipment_set." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_EQUIPMENT_SET_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    equipment_set_ref01_data = nil
    if equipment_set_ref01_data_raw.length > 0
...
def equipment_set_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "equipment_set", "EquipmentSetTestData.json")
  ...
  entid_env_raw = ENV["FLYFF_GAME_TEST_EQUIPMENT_SET_ENTID"]
