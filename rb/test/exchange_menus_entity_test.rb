...
class ExchangeMenusEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_EXCHANGE_MENUS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    exchange_menus_ref01_data_dt0_loaded = exchange_menus_ref01_ent.load(exchange_menus_ref01_match_dt0, nil)
...
def exchange_menus_basic_setup(extra)
  ...
  entid_env_raw = ENV["FLYFF_GAME_TEST_EXCHANGE_MENUS_ENTID"]
