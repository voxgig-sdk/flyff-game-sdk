...
[MCP](https://modelcontextprotocol.io) server exposing the FlyffGame SDK as
two agent tools — `flyff-game_list` and `flyff-game_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/flyff-game-mcp)
make build
...
```jsonc
// flyff-game_list: first page of records
{ "entity": "achievement" }
{ "entity": "achievement", "query": { } }

// flyff-game_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/flyff-game-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user flyff-game \
     -- "$PWD"/dist/*/flyff-game-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `flyff-game_list` and `flyff-game_load` tools now appear
   in new sessions. Ask the agent to *"list achievement using flyff-game"*
   and it calls `flyff-game_list` with `{"entity":"achievement"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `flyff-game_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `flyff-game_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
