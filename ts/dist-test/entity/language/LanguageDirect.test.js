...
(0, node_test_1.describe)('LanguageDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-language', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-language', setup.live))
            return;
        ...
        const result = await client.direct({
            path: 'image/language/{language_code}.png',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['FLYFF_GAME_TEST_LANGUAGE_ENTID'];
