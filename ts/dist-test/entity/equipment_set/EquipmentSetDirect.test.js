...
(0, node_test_1.describe)('EquipmentSetDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-equipment_set', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-equipment_set', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["equipment_set01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.equipment_set_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.equipment_set_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-equipment_set', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-equipment_set', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['FLYFF_GAME_TEST_EQUIPMENT_SET_ENTID'];
...
//# sourceMappingURL=EquipmentSetDirect.test.js.map