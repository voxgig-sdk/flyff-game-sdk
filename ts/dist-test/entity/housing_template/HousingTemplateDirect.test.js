...
(0, node_test_1.describe)('HousingTemplateDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-housing_template', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-housing_template', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["file_name01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.file_name ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.file_name = candidateId;
    ...
    (0, node_test_1.test)('direct-list-housing_template', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-housing_template', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID'];
...
//# sourceMappingURL=HousingTemplateDirect.test.js.map