...
(0, node_test_1.describe)('HousingPackDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FlyffGameSDK({
    ...
    (0, node_test_1.test)('direct-load-housing_pack', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-housing_pack', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["housing_pack01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.housing_pack_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.housing_pack_id = candidateId;
            params.housing_pack_id = setup.idmap['housing_pack01'];
        }
        else {
            params.housing_pack_id = 'direct01';
        }
        const result = await client.direct({
            path: 'housingpack/{housing_pack_id}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-housing_pack', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-housing_pack', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.FLYFF_GAME_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FlyffGameSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FLYFF_GAME_TEST_HOUSING_PACK_ENTID'];
    ...
    const client = new __1.FlyffGameSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=HousingPackDirect.test.js.map