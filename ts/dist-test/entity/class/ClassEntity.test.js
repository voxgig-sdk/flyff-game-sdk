...
(0, node_test_1.describe)('ClassEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'class.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set FLYFF_GAME_TEST_CLASS_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['class01', 'class02', 'class03', 'tree01', 'tree02', 'tree03', 'class01', 'class02', 'class03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['FLYFF_GAME_TEST_CLASS_ENTID'];
