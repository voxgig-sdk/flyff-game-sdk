...
(0, node_test_1.describe)('RecipeDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-recipe', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-recipe', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['FLYFF_GAME_TEST_RECIPE_ENTID'];
