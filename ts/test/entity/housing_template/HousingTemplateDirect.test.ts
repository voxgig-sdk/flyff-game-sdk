...
describe('HousingTemplateDirect', async () => {
  ...
  test('direct-load-housing_template', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-housing_template', setup.live)) return
    if (skipIfMissingIds(t, setup, ["file_name01"])) return
  ...
  test('direct-list-housing_template', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-housing_template', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID']
