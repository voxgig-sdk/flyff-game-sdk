...
describe('HousingTemplateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'housing_template.' + op, live)) return
    }
    ...
    const housing_template_ref01_list = await housing_template_ref01_ent.list(housing_template_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FLYFF_GAME_TEST_HOUSING_TEMPLATE_ENTID']
