...
describe('UpgradeLevelBonusDirect', async () => {
  ...
  test('direct-load-upgrade_level_bonus', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-upgrade_level_bonus', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FLYFF_GAME_TEST_UPGRADE_LEVEL_BONUS_ENTID']
