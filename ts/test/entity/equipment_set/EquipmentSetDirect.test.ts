...
describe('EquipmentSetDirect', async () => {
  ...
  test('direct-load-equipment_set', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-equipment_set', setup.live)) return
    if (skipIfMissingIds(t, setup, ["equipment_set01"])) return
  ...
  test('direct-list-equipment_set', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-equipment_set', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FLYFF_GAME_TEST_EQUIPMENT_SET_ENTID']
