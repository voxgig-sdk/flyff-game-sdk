...
describe('HousingPackEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'housing_pack.' + op, live)) return
    }
    ...
    const housing_pack_ref01_list = await housing_pack_ref01_ent.list(housing_pack_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FLYFF_GAME_TEST_HOUSING_PACK_ENTID']
