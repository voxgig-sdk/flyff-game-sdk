...
describe('HousingPackDirect', async () => {
  ...
  test('direct-load-housing_pack', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-housing_pack', setup.live)) return
    if (skipIfMissingIds(t, setup, ["housing_pack01"])) return
  ...
  test('direct-list-housing_pack', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-housing_pack', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FLYFF_GAME_TEST_HOUSING_PACK_ENTID']
