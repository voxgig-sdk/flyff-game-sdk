...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FLYFFGAME_TEST_LIVE ||
    'TRUE' === process.env.FLYFFGAME_TEST_OVERRIDE
  ) {
  ...
  m.FLYFFGAME_TEST_EXPLAIN = process.env.FLYFFGAME_TEST_EXPLAIN || m.FLYFFGAME_TEST_EXPLAIN
